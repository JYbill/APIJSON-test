...
import top.jybill.config.DemoFunctionParser;
import top.jybill.config.DemoParser;
import top.jybill.config.DemoVerifier;
import top.jybill.model.Credential;
...
public class DemoController extends APIJSONController {
	...
	public Parser<Long> newParser(HttpSession session, RequestMethod method) {
		return super.newParser(session, method).setNeedVerify(true).setNeedVerifyLogin(false).setNeedVerifyContent(true);
		//TODO
	...
	public JSONObject login(@RequestBody String request, HttpSession session) {
		...
		try {
			...
			username = requestObject.getString(KEY_ACCOUNT);
		...
		JSONObject userExistObj = new APIJSONParser(GETS, false).parseResponse(
						new JSONRequest(USER_CLASS_NAME,
								new apijson.JSONObject("").puts(KEY_ACCOUNT, username)
						)
				);
		...
		JSONObject pwdMatchObj = new APIJSONParser(GETS, false).parseResponse(
				new JSONRequest(
						...
						new apijson.JSONObject(
								new apijson.JSONObject("").puts(KEY_PASSWORD, password).puts(USER_ID, user.getId())
				))
		);
	...
	public JSONObject register(@RequestBody String request, HttpSession session) {
		JSONObject requestObject = null; // 前端传过来的JSON对象

		String account, password, nikename;

		// 提取信息
		try {
			requestObject = APIJSONParser.parseRequest(request);
			account = requestObject.getString(KEY_ACCOUNT);
			password = requestObject.getString(KEY_PASSWORD);
			nikename = requestObject.getString(KEY_NICKNAME);
		} catch (Exception e) {
			return APIJSONParser.extendErrorResult(requestObject, e);
		}
		...
		// 检查用户名冲突
		JSONObject userExistObj = new APIJSONParser(GETS, false).parseResponse(
				new JSONRequest(new User().setAccount(account)));
		...
		// 构建新用户
		User user = new User().setAccount(account).setNikename(nikename).setRole(DemoFunctionParser.USER_ROLE);
		Credential credential = new Credential().setPassword(password);
	...
	@PostMapping("reload")
	@Override
	public JSONObject reload(@RequestBody(required = false) String request) {
		JSONObject requestObject = null;
		try {
		...
		try {
			result.put(ACCESS_, DemoVerifier.initAccess(false, null,
							requestObject.getJSONObject("Access")));
			result.put(FUNCTION_, DemoFunctionParser.init(false, null,
							requestObject.getJSONObject("Function")));
			result.put(REQUEST_, DemoVerifier.initRequest(false, null,
							requestObject.getJSONObject("Request")));
